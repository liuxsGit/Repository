...
public class Client{
	...
	public void send() {
		...
		String message = textField.getText().trim();//获取用户发送的消息
		if (message == null || message.equals("")) {
			JOptionPane.showMessageDialog(frame, "消息不能为空！", "错误",
					JOptionPane.ERROR_MESSAGE);
			return;
		}
		sendMessage(frame.getTitle() + "/" + "ALL" + "/" + message);//用于实现转发消息的命令控制
//		textArea.append(frame.getTitle());
	...
	public Client() {
		...
		txt_hostIp = new JTextField("172.22.21.34");
		...
		centerSplit = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, leftScroll,
				rightScroll);//将消息区和用户列表区分开
		centerSplit.setDividerLocation(100);
 
		frame = new JFrame("客户机");
		// 更改JFrame的图标：
		frame.setIconImage(Toolkit.getDefaultToolkit().createImage(Client.class.getResource("1.jpg")));
	...
	public boolean connectServer(int port, String hostIp, String name) {
		...
		try {
			...
			sendMessage(name + "@" + socket.getLocalAddress().toString());//写入流，发送给消息处理线程
			// 开启接收消息的线程
	...
	class MessageThread extends Thread {
		...
		public void run() {
			...
			while (true) {
				try {
					message = reader.readLine();//登陆成功的系统提示消息
