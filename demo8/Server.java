...
public class Server {
	...
	public Server() {
		...
		frame.setIconImage(Toolkit.getDefaultToolkit().createImage(Server.class.getResource("317546.jpg")));
...
//				contentArea.append(inf);
				StringTokenizer st = new StringTokenizer(inf, "@");//接收到用户姓名和IP地址
//				contentArea.append(st.toString());
				user = new User(st.nextToken(), st.nextToken());//获取用户姓名和IP
				// 反馈连接成功系统信息
		...
		public void dispatcherMessage(String message) {
			...
			String source = stringTokenizer.nextToken();//发送者（客户端）
//			contentArea.append(source+"\n");
			String owner = stringTokenizer.nextToken();//ALL，所有人
//			contentArea.append(owner+"\n");
			String content = stringTokenizer.nextToken();//message被@分割之前部分
//			contentArea.append(content+"\n");
//			String si = stringTokenizer.nextToken();//实现私聊
			StringTokenizer conStringTokenizer = new StringTokenizer(content,":");//@a:dsadsad
			...
			if (owner.equals("ALL")) {// 群发
				...
				if(content.startsWith("@") && content.indexOf(":")>-1){
					String otherName = conStringTokenizer.nextToken().substring(1);
					String msgCon = conStringTokenizer.nextToken();
//					contentArea.append(otherName);
					for(ClientThread somebody:clients) {
						
						if(somebody.getUser().getName().equals(otherName)) { 
//							contentArea.append("shifouyouzuotong");
							somebody.getWriter().println(source+"对你说:"+msgCon+"(私聊)");	
							somebody.getWriter().flush();
							break;
						}
					}
				}else {
